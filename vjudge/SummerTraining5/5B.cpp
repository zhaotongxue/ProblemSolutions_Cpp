...
int main()
{
    ...
    while (res.size() < nums)
    {
        for (int i = 1; i < index; i++)
        {
            if (ds[i] == NULL)
                continue;
            if ((ds[i].back() * 2 == res.front()) || (ds[i].back() % 3 == 0 && ds[i].back() / 3 == res.front()))
            {
                for (deque<ll>::iterator it = ds[i].end(), it--; it != ds[i].begin(); it--)
                {
                    res.push_front(*it);
                }
            ...
            else if ((res.back() % 3 == 0 && res.back() / 3 == ds[i].front()) || (res.back() * 2 == ds[i].front()))
            {
                for (deque<ll>::iterator it = ds[i].begin(); it != ds[i].end(); it++)
                {
                    res.push_back(*it);
                }
                ds[i] = NULL;
