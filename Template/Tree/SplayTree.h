 ...
 * 构造函数
 */
template <class T> SplayTree<T>::SplayTree() : mRoot(NULL) {}

/*
 * 析构函数
 */
template <class T> SplayTree<T>::~SplayTree() { destroy(mRoot); }

/*
 * 前序遍历"伸展树"
 */
template <class T> void SplayTree<T>::preOrder(SplayTreeNode<T> *tree) const {
    if (tree != NULL) {
        cout << tree->key << " ";
        preOrder(tree->left);
        preOrder(tree->right);
...
template <class T> void SplayTree<T>::preOrder() { preOrder(mRoot); }

/*
 * 中序遍历"伸展树"
 */
template <class T> void SplayTree<T>::inOrder(SplayTreeNode<T> *tree) const {
    if (tree != NULL) {
        inOrder(tree->left);
        cout << tree->key << " ";
        inOrder(tree->right);
...
template <class T> void SplayTree<T>::inOrder() { inOrder(mRoot); }

/*
 * 后序遍历"伸展树"
 */
template <class T> void SplayTree<T>::postOrder(SplayTreeNode<T> *tree) const {
    if (tree != NULL) {
        postOrder(tree->left);
...
template <class T> void SplayTree<T>::postOrder() { postOrder(mRoot); }
...
template <class T> SplayTreeNode<T> *SplayTree<T>::search(T key) {
 ...
 */
template <class T>
SplayTreeNode<T> *SplayTree<T>::iterativeSearch(SplayTreeNode<T> *x,
                                                T key) const {
...
template <class T> SplayTreeNode<T> *SplayTree<T>::iterativeSearch(T key) {
...
template <class T> T SplayTree<T>::minimum() {
...
template <class T> T SplayTree<T>::maximum() {
 ...
 *      b-1
 * "键值为key的节点"的前驱节点存在的话，将"键值为key的节点"的前驱节点旋转为根节点。
 *      b-2
 * "键值为key的节点"的前驱节点存在的话，则意味着，key比树中任何键值都小，那么此时，将最小节点旋转为根节点。
 *   (c)：伸展树中不存在"键值为key的节点"，并且key > tree->key。
 *      c-1
 * "键值为key的节点"的后继节点存在的话，将"键值为key的节点"的后继节点旋转为根节点。
 *      c-2
...
template <class T> void SplayTree<T>::splay(T key) {
 ...
 */
template <class T>
SplayTreeNode<T> *SplayTree<T>::insert(SplayTreeNode<T> *&tree,
                                       SplayTreeNode<T> *z) {
...
template <class T> void SplayTree<T>::insert(T key) {
...
template <class T> void SplayTree<T>::remove(T key) {
 ...
 * 销毁伸展树
 */
template <class T> void SplayTree<T>::destroy(SplayTreeNode<T> *&tree) {
    if (tree == NULL)
        return;
...
template <class T> void SplayTree<T>::destroy() { destroy(mRoot); }
...
template <class T> void SplayTree<T>::print() {
    if (mRoot != NULL)
